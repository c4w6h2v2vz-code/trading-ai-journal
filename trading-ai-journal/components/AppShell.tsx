  ...
  return (
    <main className="min-h-screen bg-[#050505] text-white">
      <nav className="hidden md:flex sticky top-0 z-50 items-center gap-2 border-b border-white/10 bg-black/80 px-6 py-3 backdrop-blur-xl">
        ...
        <button
          onClick={logout}
          className="ml-auto rounded-xl bg-white/5 px-4 py-2 text-sm text-white/50 hover:bg-white/10"
        >
          Logout
        </button>
      </nav>
      <div className="md:hidden sticky top-0 z-50 flex items-center justify-between border-b border-white/10 bg-black/80 px-4 py-3 backdrop-blur-xl">
        ...
        <button
          onClick={logout}
          className="rounded-xl bg-white/5 px-3 py-2 text-xs text-white/50"
        >
          Logout
        </button>
      </div>
        ...
        <div className="flex items-center justify-around px-2 py-2">
          {links.map((link) => {
            const isActive =
              pathname === link.href ||
              (link.name === "More" &&
                !links.slice(0, 4).some((l) => l.href === pathname));
            if (link.name === "More") {
              return (
                <button
                  key={link.name}
                  onClick={() => setShowMenu(!showMenu)}
                  className={\`flex flex-col items-center gap-1 rounded-2xl px-3 py-2 text-xs transition \${
            ...
            return (
                ...
                href={link.href}
                className={\`flex flex-col items-center gap-1 rounded-2xl px-3 py-2 text-xs transition \${
                  pathname === link.href ? "text-blue-400" : "text-white/40"
                }\`}
              >
                <span className="text-xl">{link.icon}</span>
                <span>{link.name}</span>
      ...
      {showMenu && (
        ...
        >
          <div
            className="absolute bottom-20 left-4 right-4 rounded-3xl border border-white/10 bg-[#111] p-4"
          ...
          >
            ...
            <button
              onClick={logout}
              className="mt-3 w-full rounded-2xl border border-red-500/20 bg-red-500/10 py-3 text-sm text-red-400"
            >
              Logout
            </button>
          </div>
...
fs.writeFileSync('components/AppShell.tsx', content);
console.log('Done!');